---
name: feature
description: Run the full ContentStudio feature pipeline from idea to Shortcut delivery (research, workflow design, PRD, epic and stories, then push). Use when the user invokes /feature or $feature with a feature name and description.
---
...
- **Shortcut config:** Read `/home/casper/code/contentstudio-claude-product-pipeline/.claude/shortcut-config.json` for all Shortcut API IDs (workflows, states, groups, custom fields, projects, miscellaneous epic, iterations)
- **Config source of truth:** Always use this exact file and do not use any other Shortcut config path.
...
Read the Shortcut config from `/home/casper/code/contentstudio-claude-product-pipeline/.claude/shortcut-config.json`.
