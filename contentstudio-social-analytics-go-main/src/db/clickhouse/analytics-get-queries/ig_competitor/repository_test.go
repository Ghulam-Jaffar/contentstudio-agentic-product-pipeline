...
func newParams() *CompetitorQueryParams {
	return &CompetitorQueryParams{
		PageIDs: []string{"ig_123", "ig_456"},
		Accounts: map[string]AccountInfo{
			"ig_123": {Image: "img1.jpg", Name: "IG Page One", State: "active", Slug: "ig-page-one"},
			"ig_456": {Image: "img2.jpg", Name: "IG Page Two", State: "active", Slug: "ig-page-two"},
		},
...
func emptyMockRows() *mockRows {
	return &mockRows{
		columnNames: []string{"business_account_id"},
		columnTypes: []driver.ColumnType{&mockColumnType{name: "business_account_id", scanType: reflect.TypeOf("")}},
	}
}
...
func TestAccountFilter(t *testing.T) {
	result := accountFilter("business_account_id", []string{"ig_1", "ig_2"})
	if !strings.Contains(result, "business_account_id IN") {
		t.Fatalf("expected business_account_id IN clause, got %q", result)
	}
}
...
func TestAccountValues(t *testing.T) {
	tests := []struct {
		...
		{name: "single", ids: []string{"ig_1"}, want: "(('ig_1'))"},
...
func TestConstantConditions(t *testing.T) {
	accounts := map[string]AccountInfo{
		"ig_1": {Name: "Test Page", State: "active"},
	}
	result := constantConditions(accounts, "name", "business_account_id")
...
func TestDayMapping(t *testing.T) {
	...
	if !strings.Contains(result, "Monday") || !strings.Contains(result, "Sunday") {
		t.Fatalf("expected day names, got %q", result)
	}
}
...
func TestHourMapping(t *testing.T) {
	...
	if !strings.Contains(result, "12:00 AM") || !strings.Contains(result, "11:00 PM") {
		t.Fatalf("expected hour labels, got %q", result)
	}
}
...
func TestGetDataTableMetrics_QueryContainsExpectedClauses(t *testing.T) {
	...
	if !strings.Contains(conn.lastQuery, "instagram_competitor_posts") {
		t.Fatalf("expected posts table, got: %s", conn.lastQuery)
	}
	if !strings.Contains(conn.lastQuery, "instagram_competitor_insights") {
		t.Fatalf("expected insights table, got: %s", conn.lastQuery)
	}
	if !strings.Contains(conn.lastQuery, "ORDER BY") {
		t.Fatalf("expected ORDER BY, got: %s", conn.lastQuery)
	}
}

func TestGetDataTableMetrics_Error(t *testing.T) {
...
func TestGetPostingActivityBySpecificType_QueryContains(t *testing.T) {
	conn := &mockConn{queryRows: emptyMockRows()}
	repo := NewRepository(newTestClient(conn))
	_, err := repo.GetPostingActivityBySpecificType(context.Background(), newParams(), "IMAGE", "FEED", "followersCount")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !strings.Contains(conn.lastQuery, "IMAGE") {
		t.Fatalf("expected media_type in query, got: %s", conn.lastQuery)
	}
}
...
func TestGetPostingActivityTableByType_QueryContains(t *testing.T) {
	conn := &mockConn{queryRows: emptyMockRows()}
	repo := NewRepository(newTestClient(conn))
	_, err := repo.GetPostingActivityTableByType(context.Background(), newParams(), "IMAGE", "FEED", "followersCount")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !strings.Contains(conn.lastQuery, "instagram_competitor_posts") {
...
func TestGetPostingActivityTableByType_Error(t *testing.T) {
	conn := &mockConn{queryErr: errors.New("fail")}
	repo := NewRepository(newTestClient(conn))
	_, err := repo.GetPostingActivityTableByType(context.Background(), newParams(), "IMAGE", "FEED", "followersCount")
	if err == nil {
...
func TestGetFollowersGrowthComparison_QueryContains(t *testing.T) {
	conn := &mockConn{queryRows: emptyMockRows()}
	repo := NewRepository(newTestClient(conn))
	_, err := repo.GetFollowersGrowthComparison(context.Background(), newParams(), "total_followed_by_count")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !strings.Contains(conn.lastQuery, "instagram_competitor_insights") {
...
func TestGetTopAndLeastPerformingPosts_QueryContains(t *testing.T) {
	...
	if !strings.Contains(conn.lastQuery, "top_5_posts") {
		t.Fatalf("expected top_5_posts in query, got: %s", conn.lastQuery)
	}
	if !strings.Contains(conn.lastQuery, "least_5_posts") {
		t.Fatalf("expected least_5_posts in query, got: %s", conn.lastQuery)
	}
	if !strings.Contains(conn.lastQuery, "instagram_competitor_posts") {
		t.Fatalf("expected posts table, got: %s", conn.lastQuery)
	}
...
func TestGetTopHashtags_QueryContains(t *testing.T) {
	conn := &mockConn{queryRows: emptyMockRows()}
	repo := NewRepository(newTestClient(conn))
	_, err := repo.GetTopHashtags(context.Background(), newParams(), 10)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if !strings.Contains(conn.lastQuery, "LIMIT 10") {
		t.Fatalf("expected LIMIT 10, got: %s", conn.lastQuery)
	}
}
...
func TestGetIndividualHashtagData_QueryContains(t *testing.T) {
	conn := &mockConn{queryRows: emptyMockRows()}
	repo := NewRepository(newTestClient(conn))
	_, err := repo.GetIndividualHashtagData(context.Background(), newParams(), "travel")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !strings.Contains(conn.lastQuery, "travel") {
...
func TestQueryRows_Success(t *testing.T) {
	conn := &mockConn{
		queryRows: &mockRows{
			values:      [][]any{{"ig_123"}, {"ig_456"}},
			columnNames: []string{"business_account_id"},
			columnTypes: []driver.ColumnType{&mockColumnType{name: "business_account_id", scanType: reflect.TypeOf("")}},
		},
	}
	repo := NewRepository(newTestClient(conn))
	results, err := repo.queryRows(context.Background(), "SELECT business_account_id")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(results) != 2 {
	...
	if results[0]["business_account_id"] != "ig_123" {
		t.Fatalf("expected ig_123, got %v", results[0]["business_account_id"])
	}
}
