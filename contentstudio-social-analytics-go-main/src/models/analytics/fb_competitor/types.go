// Package fb_competitor provides request/response types for Facebook competitor analytics.
...
// CompetitorRequest holds the parsed query parameters for competitor analytics endpoints.
type CompetitorRequest struct {
	...
	// Single-competitor endpoints (postReactDistribution, postEngagementOverTime)
...
func (r *CompetitorRequest) Validate() error {
	if r.ReportID == "" && r.FacebookID == "" {
		return httputil.NewValidationError("_id (report_id) or facebook_id is required")
	}
...
// Start date gets 00:00:01, end date gets 23:59:59 in the user timezone, then converted to UTC.
// Returns (startUTC, endUTC, daysDiff).
func (r *CompetitorRequest) ToUTCDateRange() (string, string, int, error) {
	...
	startLocal = startLocal.Add(1 * time.Second) // 00:00:01
