...
import (
	...
	"github.com/d4interactive/contentstudio-social-analytics-go/src/services/instagram/instagram-competitor-analysis/service"
...
const (
	PlatformTypeInstagram = "instagram"
	TokenQueueInstagram   = "instagram_valid_token_set"
	WorkersPerPool        = 10
)

// CompetitorJob represents a single Instagram competitor job
type CompetitorJob struct {
	ReportID  string // MongoDB report ID
	PageID    string // Instagram page ID
	CompID    string
	Mode      apiModels.SyncMode
	StartDate string
	EndDate   string
}
...
func main() {
	...
	log.Info().Msg("Starting Instagram Competitor Analysis Service")

	rootOp := log.Operation("instagram_competitor_service").
		WithSentryTags(map[string]string{
			"platform": PlatformTypeInstagram,
		})
	...
	chRepo, _ := clickhouseRepo.NewClient(cfg.ClickHouse, log.Logger)

	// Kafka consumers
	consumerRealtime, err := kafka2.NewConsumer(
		cfg.Kafka,
		"instagram-competitor-realtime",
		log.Logger,
	)
	if err != nil {
	...
	startParsePool(ctx, &wg, "realtime-parse", realtimeFetchResults, realtimeParseResults, log)
	...
	startParsePool(ctx, &wg, "batch-parse", batchFetchResults, batchParseResults, log)
	...
	log.Info().Msg("Instagram competitor service stopped")
	logger.FlushSentry(5 * time.Second)
}
	...
	for i := 0; i < WorkersPerPool; i++ {
		...
		go func(workerID int) {
			...
			svc := service.NewCompetitorAnalysisService(
				social.NewInstagramClientWithRates(cfg.Facebook.AppSecret, rateMgr),
				mongoRepo,
...
func startParsePool(
	...
	for i := 0; i < WorkersPerPool; i++ {
		...
		go func(workerID int) {
			...
			svc := service.NewCompetitorAnalysisService(
				nil, // Instagram client not needed for parse stage
...
func startStorePool(
	...
	for i := 0; i < WorkersPerPool; i++ {
		...
		go func(workerID int) {
			...
			svc := service.NewCompetitorAnalysisService(
				nil, // Instagram client not needed for store stage
				mongoRepo,
				chRepo,
				log,
			)
...
func sendCompetitorNotifications(
	...
	sendInstagramPusherNotification(pusherClient, competitor, report, pageID, parseResult, log)
	...
	if (parseResult.CurrentState == "Added" || parseResult.CurrentState == "NotFound") && report != nil {
		sendInstagramEmailNotification(ctx, notifier, mongoRepo, report, competitor, parseResult, log)
	}
}

// sendInstagramPusherNotification sends real-time notification via Pusher
func sendInstagramPusherNotification(
	pusherClient *notification.PusherClient,
	...
	channel := fmt.Sprintf("ig-competitor-analytics-%s", report.WorkspaceID.Hex())
	event := fmt.Sprintf("ig-competitor-analytics-%s", report.WorkspaceID.Hex())
	...
	data := map[string]interface{}{
		...
		"display_name": parseResult.Payload.DisplayName,
	...
	if err := pusherClient.Trigger(channel, event, data); err != nil {
		log.Error().Err(err).Str("error_message", err.Error()).Str("channel", channel).Str("function", "sendInstagramPusherNotification").Str("stage", "pusher_trigger").Msg("Failed to send Pusher notification")
	} else {
		log.Info().Str("channel", channel).Msg("Pusher notification sent successfully")
	}
}

// sendInstagramEmailNotification sends email notification to user via backend
func sendInstagramEmailNotification(
	ctx context.Context,
	notifier *notification.Service,
	mongoRepo *mongoRepo.CompetitorRepository,
	report *mongoModels.CompetitorReport,
	...
	if accountName == "" {
		if parseResult != nil && parseResult.Payload != nil {
			accountName = parseResult.Payload.DisplayName
		}
	}
	...
	if err != nil {
		log.Error().Err(err).Str("error_message", err.Error()).Str("userId", user.ID.Hex()).Str("competitorId", accountID).Str("function", "sendInstagramEmailNotification").Str("stage", "send_notification").Msg("Failed to send analytics notification to backend")
	} else {
...
func buildPayload(
	...
	cfg *config.Config,
	log *logger.Logger,
) *apiModels.InstagramCompetitorPayload {
	...
	token, igBusinessID, err := getCompetitorToken(ctx, redis, cfg)
	if err != nil {
	...
	return &apiModels.InstagramCompetitorPayload{
...
func consumeRealtime(ctx context.Context, consumer kafka2.Consumer, jobs chan<- CompetitorJob, log *logger.Logger) {
	log.Info().Msg("Starting Instagram realtime consumer")
	consumer.Consume(ctx, []string{"competitor-work-order-instagram"}, func(_ context.Context, _ string, _, value []byte) error {
		...
		select {
		case jobs <- CompetitorJob{ReportID: reportID, PageID: pageID, CompID: pageID, Mode: apiModels.SyncMode(wo.Mode), StartDate: wo.StartDate, EndDate: wo.EndDate}:
		case <-ctx.Done():
			return ctx.Err()
		}
...
func consumeBatch(ctx context.Context, consumer kafka2.Consumer, jobs chan<- CompetitorJob, log *logger.Logger) {
	log.Info().Msg("Starting Instagram batch consumer")
	consumer.Consume(ctx, []string{"competitor-work-order-instagram-batch"}, func(_ context.Context, _ string, _, value []byte) error {
...
// getCompetitorToken retrieves and decrypts an Instagram token from Redis
func getCompetitorToken(ctx context.Context, redisClient *redis.Client, cfg *config.Config) (string, string, error) {
	candidate, err := competitortokens.FetchCandidate(ctx, redisClient, TokenQueueInstagram, cfg.DecryptionKey, true, nil)
	if err != nil {
		return "", "", err
	}
...
func fetchWithTokenRetry(
	...
	if result.Error == nil || !competitortokens.IsInstagramTokenIssue(result.Error) {
		return result
	}

	exclude := map[string]struct{}{
		competitortokens.CandidateKey(payload.AccessToken, payload.BusinessID): {},
	}
	candidate, err := competitortokens.FetchCandidate(ctx, redisClient, TokenQueueInstagram, cfg.DecryptionKey, true, exclude)
	if err != nil {
		log.Warn().Err(err).Str("page_id", payload.PageID).Msg("No fresh Instagram competitor token available for retry")
		return result
	}

	retryPayload := *payload
	retryPayload.AccessToken = candidate.AccessToken
	retryPayload.BusinessID = candidate.PlatformID

	log.Warn().Str("page_id", payload.PageID).Msg("Retrying Instagram competitor fetch with a fresh token")
