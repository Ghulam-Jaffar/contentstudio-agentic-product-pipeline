...
func main() {
	...
	log.Info().Msg("Starting LinkedIn Immediate Processor")
	...
	consumer, err := kafka2.NewConsumer(cfg.Kafka, "linkedin-immediate-processor-group", log.Logger)
	if err != nil {
	...
	go func() {
		log.Info().Msg("Consuming immediate-work-order-linkedin topic")
		err = consumer.Consume(ctx, []string{"immediate-work-order-linkedin"}, func(ctx context.Context, _ string, _ []byte, value []byte) error {
			select {
	...
	wg.Wait()
	log.Info().Msg("LinkedIn Immediate Processor stopped")
}
...
func worker(workerID int, workChan <-chan workMessage, proc *processor.Processor, log *logger.Logger) {
	...
	for msg := range workChan {
		...
		log.Info().
			...
			Msg("Starting immediate processing for LinkedIn account")
		...
		if err != nil {
			...
			log.Info().
				...
				Str("workspace_id", wo.WorkspaceID).
				Dur("duration", duration).
				Msg("Successfully processed LinkedIn account")
		}
	}
}
