...
These rules apply to every story generated by the feature and story pipelines. Follow them exactly. The pipelines produce stories as local markdown and push to nothing. A Product Owner reads the markdown and recreates each story in the team's tracker by hand.

---

## 1. Story Template

**Structure every story with the sections in `docs/story-template.md`:** Description, Workflow, Acceptance criteria, Mock-ups, Impact on existing data, Impact on other products, Dependencies, and the global quality checklist.

A story **ends at the global quality checklist**. Never append a trailing metadata block — no story type, project, group, epic, priority, product area, skill set, estimate, labels, or iteration. The pipeline pushes nothing, so there is no API payload to populate, and the Product Owner sets all of that when creating the story in the tracker.
...
**Implementation pointers do not belong in Workflow, or anywhere else in the story.** File paths, class names, method signatures, helper suggestions, JWT/Redis/cache mechanics, ORM details — none of this goes in the story at all. If it's useful to the dev, it goes in the local research doc (see section 16).
...
A code reference is fine when it disambiguates *which* behavior the AC is testing ("Compose dropdown excludes the Blog post action"). It's wrong when it dictates *how* to implement. If it's the latter, rewrite it as observable behavior or move it to the research doc (section 16).
...
**Never add story point estimates** to stories. Estimation is done by the development team during sprint planning, not by the story author.

---

## 12. No Labels on Stories

Do not add labels (like "New") to stories. The team manages labels themselves in the tracker.

---

## 13. Mobile App Stories
...
## 14. No Local File References in Story Content

**Never include local file paths from this pipeline project** (e.g., `docs/features/whatsapp-inbox-integration/01-research.md`) in story descriptions, links, or references. The story content is written for whoever reads it in the tracker once the PO creates it there — developers, designers, QA — and they have no access to this repo's local files.

Instead:
- **Reference other stories** by their full title. Tracker URLs don't exist yet (the pipeline doesn't push), so titles are the only stable handle.
...
- **Codebase file paths** (e.g., `contentstudio-frontend/src/modules/...`) do not belong in story content either. They refer to real product code, so they are useful — but they live in the local research doc, not the story (see section 16).
...
**Good:** put the concrete codebase pointer in the local research doc — see section 16

### Attachments and image previews

When screenshots or mockups are attached to a story, **embed images using Markdown image syntax** so the tracker renders an inline preview:

```
![image.png](<attachment-url>)
```
...
- Stories don't exist in a tracker at implementation time, so there is no ticket ID to put in the branch name.
...
- **Description:** Summary of changes, files modified, and the story title(s) implemented
- **Base branch:** `develop`

---

## 16. No Implementation References in Stories

**Do not add an "Implementation references" section to any story.** Stories stay clean: Description, Workflow, Acceptance criteria, Mock-ups, Impact on existing data, Impact on other products, Dependencies, and the global quality checklist. Nothing after the checklist.

This supersedes the earlier rule that mandated a trailing Implementation references block.

### Where the pointers go instead

Research still surfaces genuinely useful things — codebase entry points, existing patterns, suggested names, gotchas, latent bugs. Those belong in the local research doc, `docs/stories/<slug>/01-research.md` (or `docs/features/<slug>/01-research.md`), which stays local and is never recreated in the tracker.

That keeps the value without putting file paths, class names and line numbers into a story that designers, QA and the PO read top to bottom.

### What this means for the other sections

- **Workflow** stays user-POV. Implementation detail does not move to the end of the story, it moves to the research doc.
- **Acceptance criteria** describe observable behaviour. If an AC is really a "how", rewrite it as behaviour or drop it to the research doc.
- **Codebase paths** do not appear in story content at all. They live in the research doc.

### Why

The story is the shared artifact. A trailing wall of paths and line numbers makes it longer, dates faster, and reads as a spec rather than a statement of intent. The research doc already exists for exactly this and stays local.

---

## 17. Analytics Event Tracking (Usermaven)
...
## 18. Diagrams in Workflow Sections
...
- Always wrap in a fenced code block tagged `mermaid` so the tracker and GitHub render it
